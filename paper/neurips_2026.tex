...
Standard offline GCRL implementations typically use feedforward MLPs to produce the value, critic, or compatibility signal consumed by the actor. This design is simple and effective, but it gives the learned signal a fixed amount of computation before policy extraction. At the same time, architecture has become an increasingly important lever in reinforcement learning, with depth, width, regularization, residual structure, and explicit compute scaling improving several online, offline, and self-supervised settings \citep{resnet, d2rl, dr3, bro, simba, thousand_layers}. These observations motivate a narrower architectural question: rather than only increasing the size of a feedforward critic, can we improve the actor-facing signal by allocating computation through repeated refinement of the value representation?


We study recurrent latent refinement for offline GCRL value learning. Instead of producing a value or compatibility score in one feedforward pass, we let it repeatedly revises a latent representation before exposing the final signal. This follows a broader line of models that allocate computation through repeated latent updates or implicit depth \citep{universal_transformer, act, deq, recurrent_relational_networks, energy_diffusion, hrm, trm, urm}. As illustrated in Figure~\ref{fig:refinement-simple}, our module applies a sequence of refinement steps to an initial latent state; Section~\ref{sec:architecture} gives the detailed design, including SwiGLU updates, step embeddings, LayerNorm, and LayerScale \citep{shazeer_glu, universal_transformer, layer_norm, layerscale}. We insert this module as a drop-in replacement for the MLP value or critic backbone in existing offline GCRL algorithms, keeping losses, data, actors, optimization unchanged. We use Contrastive RL (CRL) as the main diagnostic case because its actor directly consumes the learned compatibility score \citep{crl}, allowing us to measure changes in the actor-facing signal.
...
\caption{Main results on OGBench tasks. Scores are normalized average binary success rates in percentage points across 8 seeds; numbers after $\pm$ denote standard deviations. CRL and HIQL baselines are taken from OGBench \citep{ogbench}, and SAW baselines are taken from the SAW paper \citep{saw}. Following OGBench, bold denotes scores at or above 95\% of the best mean in each row.}
\label{tab:main_results}
\end{table*}

The table uses the default recurrent configurations selected for each algorithm; the roles of refinement depth $K$ and per-step update capacity $m$ are studied next in Section~\ref{sec:depth} and Section~\ref{sec:update-capacity}.


\subsection{Effect of Refinement Depth}
\label{sec:depth}
The main results use a fixed refinement depth, but the benefit of recurrence should depend on whether repeated updates provide useful additional computation. We therefore sweep the number of outer refinement steps $K$ in CRL on \texttt{Antmaze-Medium-Stitch} (AMS) and \texttt{Antmaze-Giant-Navigate} (AGN), comparing the MLP baseline with recurrent critics using $K\in\{1,2,4,6,8\}$. Figure~\ref{fig:iter_ams_agn} shows a clear depth-dependent pattern. A single recurrent update is insufficient, indicating that the gain is not explained merely by replacing the MLP block with a different parameterization. Increasing the number of refinement steps produces a sharp improvement, but the gains then saturate rather than increasing monotonically, suggesting that recurrent refinement is useful up to the point where the latent representation has resolved the relevant reachability structure, after which additional updates provide limited benefit and can introduce optimization noise. We use $K=4$ in the main comparisons because it lies in this high-performing regime while keeping training and evaluation cost moderate. Detailed numeric results for the depth sweep are reported in the appendix.
...
\begin{figure}[h]
    ...
    \caption{Learning curves for refinement-depth sweeps on AMS and AGN. Moderate recurrent depth improves performance substantially over one-step recurrent critic, while further increases show diminishing returns. Lines and shaded regions indicate average success rate and standard deviation over four seeds.}
...
Across AMS, ALS, and AGN, recurrent refinement improves normalized separation between matched and mismatched goals (Figure~\ref{fig:success_sep_graphcorr}(a,b); full statistics in appendix). The critic-side effect is therefore more specific than score scaling: recurrence changes the distribution of scores in a way that improves normalized positive-negative discrimination.
...
\begin{figure}[!ht]
    ...
    \begin{minipage}[t]{0.35\linewidth}
        ...
        \begin{subfigure}[t]{\linewidth}
            \includegraphics[width=\linewidth,trim=7pt 7pt 7pt 7pt,clip]{figures/mse.pdf}
            \caption{Policy MSE}
            \label{fig:mse}
        \end{subfigure}
        \vspace{0.35em}
        \begin{subfigure}[t]{\linewidth}
            \includegraphics[width=\linewidth,trim=7pt 7pt 7pt 7pt,clip]{figures/qdelta.pdf}
            \caption{$q_\Delta$}
            \label{fig:qdelta}
        \end{subfigure}
    \end{minipage}
    ...
    \begin{subfigure}[t]{0.62\linewidth}
        \vspace{0pt}
        \includegraphics[width=\linewidth,height=0.415\textheight,keepaspectratio,trim=7pt 7pt 7pt 7pt,clip]{figures/ams_trajectory_bottleneck.pdf}
        \caption{AMS bottleneck trajectories}
    \end{subfigure}
    \caption{Actor-interface and trajectory diagnostics. Recurrent refinement reduces policy deviation and actor-over-dataset critic gaps while producing trajectories that traverse bottleneck regions more reliably.}
...
\begin{wrapfigure}{r}{0.48\textwidth}
    ...
    \vspace{-1.2em}
\end{wrapfigure}
The two controls are a wider three-layer critic with hidden size \texttt{688} and a deeper six-layer critic with hidden size \texttt{512}; their parameter counts are within 0.37\% and 0.19\% of the recurrent critic, respectively. As shown in Figure~\ref{fig:parameter_match}, neither feedforward alternative closes the gap to the recurrent critic. The gain is therefore not explained by parameter count alone. Instead, the result supports a compute-allocation view: recurrent refinement reuses a shared update over multiple latent steps, whereas the dense controls spend comparable capacity in a single feedforward computation. Appendix~\ref{app:compute-comparison} reports the corresponding wall-clock overhead for the recurrent critic.
...
\midrule
\texttt{Antmaze-Medium-Stitch} & MLP & 3.79 & 5.00 $\pm$ 0.33 & 200.8 $\pm$ 13.1 & 1.44 $\pm$ 0.10 & 50.6 $\pm$ 10.1 \\
\texttt{Antmaze-Medium-Stitch} & Recurrent & 8.06 & 16.60 $\pm$ 4.06 & 63.2 $\pm$ 16.4 & 4.79 $\pm$ 1.17 & 88.3 $\pm$ 5.0 \\
\texttt{Antmaze-Large-Stitch} & MLP & 3.79 & 5.61 $\pm$ 0.11 & 178.3 $\pm$ 3.6 & 1.61 $\pm$ 0.04 & 14.8 $\pm$ 1.1 \\
\texttt{Antmaze-Large-Stitch} & Recurrent & 5.89 & 12.12 $\pm$ 0.41 & 82.6 $\pm$ 2.8 & 3.43 $\pm$ 0.11 & 31.3 $\pm$ 9.0 \\
\texttt{Antmaze-Giant-Navigate} & MLP & 3.79 & 5.71 $\pm$ 0.11 & 175.3 $\pm$ 3.4 & 1.64 $\pm$ 0.03 & 12.4 $\pm$ 3.5 \\
\texttt{Antmaze-Giant-Navigate} & Recurrent & 5.89 & 11.13 $\pm$ 2.27 & 93.1 $\pm$ 21.6 & 3.14 $\pm$ 0.63 & 22.9 $\pm$ 3.6 \\
\bottomrule
\end{tabular}
}
\end{table}

The recurrent critic is therefore approximately $3.3\times$, $2.2\times$, and $2.0\times$ slower per update on AMS, ALS, and AGN, respectively. However, under the same wall-clock budget as the completed MLP baseline, the recurrent critic already achieves higher success on all three tasks. This overhead is expected because recurrent refinement applies multiple sequential latent updates before the critic emits its score. The parameter-matched width and depth controls in Figure~\ref{fig:parameter_match} are not included in this wall-clock table because their available logs were produced under different metadata, including different hostnames, a different git commit, and a different evaluation-device setting.
