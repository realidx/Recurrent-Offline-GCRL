...
sbatch --job-name=hiql_geom_eval --partition=gpu-long --time=05:00:00 --cpus-per-task=8 --mem=24G --output=logs/phase_4/slurm-%x-%j.out --error=logs/phase_4/slurm-%x-%j.err --wrap='cd "$SLURM_SUBMIT_DIR/third_party/ogbench/impls" && export OGBENCH_DATASET_DIR="$SLURM_SUBMIT_DIR/.ogbench_data" && export JAX_PLATFORMS=cpu && conda run -n recurrent python analyze_hiql_value_geometry.py --run_dir "$SLURM_SUBMIT_DIR/exp/OGBench/HIQL_AntLargeStitch_4x6/sd000_antmaze_large_stitch_hiql_recur4x6_mlpactor" --epochs 800000 --task_ids 1,2,3,4,5 --policy_rollout_steps 25 --reset_eval_episodes 50 --output_dir "$SLURM_SUBMIT_DIR/exp/value_geometry_hiql_antlarge_4x6_sd000_ckpt800000"'

sbatch --job-name=hiql_geom_eval --partition=gpu-long --time=05:00:00 --cpus-per-task=8 --mem=24G --output=logs/phase_4/slurm-%x-%j.out --error=logs/phase_4/slurm-%x-%j.err --wrap='cd "$SLURM_SUBMIT_DIR/third_party/ogbench/impls" && export OGBENCH_DATASET_DIR="$SLURM_SUBMIT_DIR/.ogbench_data" && export JAX_PLATFORMS=cpu && conda run -n recurrent python analyze_hiql_value_geometry.py --run_dir "$SLURM_SUBMIT_DIR/exp/OGBench/HIQL_AntLargeStitch_Baseline/sd000_antmaze_large_stitch_hiql_mlp_baseline" --epochs 1000000 --task_ids 1,2,3,4,5 --policy_rollout_steps 25 --reset_eval_episodes 50 --output_dir "$SLURM_SUBMIT_DIR/exp/value_geometry_hiql_baseline_sd000_ckpt1000000"'
