...
class GCDataset:
    ...
    def sample(self, batch_size, idxs=None, evaluation=False):
        ...
        batch['idxs'] = np.asarray(idxs, dtype=np.int32)
        if self.config['frame_stack'] is not None:
            batch['observations'] = self.get_observations(idxs)
            batch['next_observations'] = self.get_observations(idxs + 1)

        value_goal_idxs, value_goal_meta = self.sample_goals(
            idxs,
        ...
        actor_goal_idxs, actor_goal_meta = self.sample_goals(
            idxs,
        ...
        batch.update(self._goal_metadata('value', value_goal_idxs, value_goal_meta))
        batch.update(self._goal_metadata('actor', actor_goal_idxs, actor_goal_meta))
    ...
    def sample_goals(self, idxs, p_curgoal, p_trajgoal, p_randomgoal, geom_sample, min_d=0, return_metadata=False):
        ...
        if p_curgoal == 1.0:
            goal_idxs = idxs
            goal_source = np.zeros(batch_size, dtype=np.int32)
        else:
            choose_traj = np.random.rand(batch_size) < p_trajgoal / (1.0 - p_curgoal)
            goal_idxs = np.where(choose_traj, traj_goal_idxs, random_goal_idxs)
            goal_source = np.where(choose_traj, 1, 2).astype(np.int32)

            # Goals at the current state.
            choose_current = np.random.rand(batch_size) < p_curgoal
            goal_idxs = np.where(choose_current, idxs, goal_idxs)
            goal_source = np.where(choose_current, 0, goal_source).astype(np.int32)
        ...
        metadata = dict(
            source=goal_source,
            horizon=np.where(goal_source == 2, -1, np.maximum(goal_idxs - idxs, 0)).astype(np.int32),
        )
        return goal_idxs, metadata

    def _goal_metadata(self, prefix, goal_idxs, metadata):
        return {
            f'{prefix}_goal_idxs': np.asarray(goal_idxs, dtype=np.int32),
            f'{prefix}_goal_source': np.asarray(metadata['source'], dtype=np.int32),
            f'{prefix}_goal_horizon': np.asarray(metadata['horizon'], dtype=np.int32),
        }
...
class HGCDataset(GCDataset):
    ...
    def sample(self, batch_size, idxs=None, evaluation=False):
        ...
        batch['low_actor_goal_idxs'] = np.asarray(low_goal_idxs, dtype=np.int32)
        batch['low_actor_goal_horizon'] = np.asarray(np.maximum(low_goal_idxs - idxs, 0), dtype=np.int32)
        ...
        batch['high_actor_goal_source'] = np.where(pick_random, 2, 1).astype(np.int32)
        batch['high_actor_goal_horizon'] = np.where(
            pick_random,
            -1,
            np.maximum(high_goal_idxs - idxs, 0),
        ).astype(np.int32)
