...
class CsvLogger:
    ...
    def _rewrite_with_new_header(self, new_header):
        old_header = list(self.header or [])
        existing_rows = []
    ...
    def log(self, row, step):
        ...
        if new_keys:
            self._rewrite_with_new_header(self.header + new_keys)

        self.file.write(','.join([self._encode_csv_value(filtered_row.get(k, '')) for k in self.header]) + '\n')
        self.file.flush()
