...
class HIQLAgent(flax.struct.PyTreeNode):
    ...
    def value_loss(self, batch, grad_params):
        ...
        This value loss uses expectile regression with additional tricks to stabilize training.
    ...
    def create(
        ...
        if value_backbone == 'mlp':
            ...
            value_def = GCRecurrentValue(gc_encoder=value_encoder_def, **value_kwargs)
            target_value_def = GCRecurrentValue(gc_encoder=target_value_encoder_def, **value_kwargs)
        else:
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            value_k=4,  # Recurrent iterations K when value_backbone='recur'.
            value_m=2,  # Residual SwiGLU layers per recurrent iteration when value_backbone='recur'.
            value_layerscale_init=1e-2,  # Per-step LayerScale init for recurrent residual updates.
            discount=0.99,  # Discount factor.
            tau=0.005,  # Target network update rate.
            expectile=0.7,  # Expectile coefficient.
